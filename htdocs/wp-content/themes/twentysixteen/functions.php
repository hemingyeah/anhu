...
自定义wordpress样式
*/
//前台页面去除admin_bar组件  独孤宇云 2016-9-10
...
function Limit_Char($max_char = 1000, $more_text = '...', $more_link_text = '', $limit_type = 'content') {
...
function par_pagenavi($range = 3){
    ...
    echo "<em>共 " . $postcount->publish . " 条新闻</em><span>";
    // if($paged != 1){echo "<a href='" . get_pagenum_link(1) . "' class='wd' title='跳转到首页'>«</a>";}
    previous_posts_link('上一页');
    if($max_page > $range){
        if($paged < $range){for($i = 1; $i <= ($range + 1); $i++){echo "<span><a href='" . get_pagenum_link($i) ."'";
    ...
    if($i==$paged)echo " class='active wd'";echo ">$i</a>";}}
    next_posts_link('下一页');
    echo "</span>";
}
}
