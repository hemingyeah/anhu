...
/*对模板进行修改了,加了类"wd" 独孤宇云 2016-9-12*/
function get_next_posts_link( $label = null, $max_page = 0 ) {
	...
	if ( !is_single() && ( $nextpage <= $max_page ) ) {
		...
		return '<a class="wd" href="' . next_posts( $max_page, false ) . "\" $attr>" . preg_replace('/&([^#])(?![a-z]{1,8};)/i', '&#038;$1', $label) . '</a>';
	}
}
...
function get_previous_posts_link( $label = null ) {
	...
	if ( !is_single() && $paged > 1 ) {
		...
		return '<a class="wd" href="' . previous_posts( false ) . "\" $attr>". preg_replace( '/&([^#])(?![a-z]{1,8};)/i', '&#038;$1', $label ) .'</a>';
	}
}
